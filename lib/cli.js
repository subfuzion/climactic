...
CLI.prototype.start = function (argv) {
  ...
  if (argv.length < 3) {
    io.printBanner(this.spec.banner || 'CLI', fmt('CLI v%s, Node %s', this.spec.version, process.version));
    commander.help();
  }
...
function loadConfig() {
  ...
  if (this.spec.appconfig) {
    configFiles.push({
      file: path.basename(this.spec.appconfig),
      path: path.dirname(this.spec.appconfig)
    });
  }

  if (this.spec.userconfig) {
    configFiles.push({
      file: path.basename(this.spec.userconfig),
      path: path.dirname(this.spec.userconfig)
    });
    configFiles.push({
      file: path.basename(this.spec.userconfig),
      path: osenv.home()
  ...
  if (config.theme) {
    ...
    themepath = config.themepath
        ? path.resolve(this.spec.approot || process.cwd(), config.themepath)
